...
export const calculateTrend = (transactions) => {
  if (transactions.length === 0) return {
    incomeTrend: null, expenseTrend: null, balanceTrend: null
  }
  ...
  const getIncome   = (txs) => txs.filter(t => t.type === 'income') .reduce((a, t) => a + t.amount, 0)
  const getExpenses = (txs) => txs.filter(t => t.type === 'expense').reduce((a, t) => a + t.amount, 0)
